...
export default function App() {
  ...
  const [composeForm, setComposeForm] = useState({ to: '', subject: '', content: '', fromAccountId: '' });
  ...
  // Account Form
  const [accountForm, setAccountForm] = useState({ email: '', password: '', host: '', port: 993, protocol: 'imap' });
  ...
  const handleTestAndConnectAccount = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        setAccountForm({ email: '', password: '', host: '', port: 993, protocol: 'imap' });
      }, 1500);
  ...
  const fillPreset = (provider: 'gmail' | 'outlook' | 'zoho' | 'sendgrid') => {
    if (provider === 'gmail') setAccountForm(p => ({ ...p, host: 'imap.gmail.com', port: 993, protocol: 'imap' }));
    if (provider === 'outlook') setAccountForm(p => ({ ...p, host: 'outlook.office365.com', port: 993, protocol: 'imap' }));
    if (provider === 'zoho') setAccountForm(p => ({ ...p, host: 'imap.zoho.com', port: 993, protocol: 'imap' }));
    if (provider === 'sendgrid') setAccountForm(p => ({ ...p, host: 'api.sendgrid.com', port: 443, protocol: 'sendgrid' }));
  };
  ...
  const handleSendEmail = async () => {
    ...
    const accountId = composeForm.fromAccountId || state.accounts[0]?.id;
    if (!accountId) {
      alert("No email account connected. Please link a node first.");
      return;
    }
    ...
    try {
      ...
      setComposeForm({ to: '', subject: '', content: '', fromAccountId: '' });
        ...
        {accountStatus === 'idle' || accountStatus === 'failed' ? (
          <div className="space-y-4">
            ...
            <div className="flex gap-2 mb-4">
              ...
              <button onClick={() => fillPreset('sendgrid')} className="flex-1 py-2 bg-slate-800 rounded-lg text-[10px] font-bold uppercase hover:bg-emerald-900 text-emerald-500 border border-emerald-500/30">SendGrid</button>
            </div>

            <input type="email" placeholder={accountForm.protocol === 'sendgrid' ? "From Email (sender@domain.com)" : "email@customdomain.com"} className="w-full p-4 rounded-xl bg-[#1a1a1a] border border-slate-800 text-white" value={accountForm.email} onChange={e => setAccountForm({ ...accountForm, email: e.target.value })} />
            <div className="relative">
              <input type="password" placeholder={accountForm.protocol === 'sendgrid' ? "SendGrid API Key (SG...)" : "App Password"} className="w-full p-4 rounded-xl bg-[#1a1a1a] border border-slate-800 text-white" value={accountForm.password} onChange={e => setAccountForm({ ...accountForm, password: e.target.value })} />
              <div className="absolute right-4 top-4 text-[10px] text-slate-500">{accountForm.protocol === 'sendgrid' ? "API Key Required" : "For Gmail/Outlook use App Password"}</div>
            </div>
      ...
      <Modal isOpen={showComposeModal} onClose={() => setShowComposeModal(false)} title="New Transmission">
        <div className="space-y-6">
          ...
          {state.accounts.length > 0 && (
            <div>
              <label className="text-[10px] font-bold uppercase text-slate-500 mb-2 block">From Node</label>
              <select
                className="w-full p-4 rounded-xl bg-[#1a1a1a] border border-slate-800 text-white outline-none focus:border-emerald-500"
                value={composeForm.fromAccountId}
                onChange={e => setComposeForm({ ...composeForm, fromAccountId: e.target.value })}
              >
          ...
          <button className="w-full py-4 bg-emerald-600 text-white rounded-xl font-bold uppercase tracking-widest hover:bg-emerald-500 transition-colors" disabled={isSending} onClick={handleSendEmail}>
